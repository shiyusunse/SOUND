...
class LineDP_mixedsort(BaseModel):
    ...
    def line_level_prediction(self):
        ...
        predicted_lines_initial, predicted_score_initial = [], []
        ...
        print(f'defecive files: {len(defective_file_index)}')
        print(f'clean files: {len(clean_file_index)}')
        ...
        for i in range(len(clean_file_index)):
            print(f'{i}/{len(clean_file_index)}')
            ...
            exp = explainer.explain_instance(' '.join(clean_file_line_list), c.predict_proba, num_features=100)
            risky_tokens = [x[0] for x in exp.as_list() if x[1] > 0][:20]
        ...
        indexed_lst = [(i, x) for i, x in enumerate(predicted_score_clean)]
        sorted_lst_1 = sorted(indexed_lst, key=lambda x: (-x[1], x[0]))
        indexes = [x[0] for x in sorted_lst_1]

        for i in range(len(indexes)):
            predicted_lines.extend([predicted_lines_clean[indexes[i]]])
            predicted_score.extend([predicted_score_clean[indexes[i]]])
