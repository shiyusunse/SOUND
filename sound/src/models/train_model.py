...
def train_model(dataset_name):
    ...
    for i, train_rel in enumerate(releases[:-1]):
        df = get_df(train_rel)

        for run in range(5):
            print('#'*20, train_rel, 'run', run, 'begin!', '#'*20)

            actual_save_model_dir = save_model_dir + dataset_name + '/' + train_rel + '/' + str(run) + '/'
            loss_dir = '../output/loss/DeepLineDP/' + train_rel + '/' + str(run) + '/'
            ...
            train_df, valid_df = train_test_split(df, test_size=0.5, random_state=run)
            ...
            print('load Word2Vec for', train_rel, run, 'finished')
            ...
            if total_checkpoints == 0:
                ...
                loss_df = pd.read_csv(loss_dir+train_rel+'-loss_record.csv')
                train_loss_all_epochs = list(loss_df['train_loss'])
                val_loss_all_epochs = list(loss_df['valid_loss'])

                current_checkpoint_num = current_checkpoint_num+1
            ...
            for epoch in tqdm(range(current_checkpoint_num, num_epochs+1)):
                ...
                if epoch % save_every_epochs == 0:
                    print(train_rel,'- at epoch:',str(epoch))
                ...
                loss_df.to_csv(loss_dir+train_rel+'-loss_record.csv',index=False)
