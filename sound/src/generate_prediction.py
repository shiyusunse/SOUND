...
from sound.src.models.DeepLineDP_model import *
...
def predict_defective_files_in_releases(dataset_name, target_epochs):
    ...
    for i in range(len(releases) - 1):
        ...
        for run in range(5):
            ...
            actual_intermediate_output_dir = intermediate_output_dir+rel+'/'+str(run)+'/'
            ...
            df.to_csv(prediction_dir+'/'+str(run)+'/'+rel+'.csv', index=False)
