...
def process_csv(input_file, output_file):
    with open(input_file, 'r', encoding='utf-8', errors='ignore') as file:
        ...
        for line in lines:
            ...
            if ',' in temp[2]:
                print(temp[2])
                new_part_3 = '"' + temp[2].strip() + '"'
                new_lines.append(temp[0]+','+temp[1]+','+new_part_3)
            else:
...
for release in releases:
    process_csv('../datasets/original/Line-level/'+release+'_defective_lines_dataset.csv',
                '../datasets/original/Line-level/'+release+'_defective_lines_dataset.csv')
