...
def preprocess_data(proj_name):
    ...
    for rel in cur_all_rel:
        print(file_lvl_dir+rel+'_ground-truth-files_dataset.csv')
        file_level_data = pd.read_csv(file_lvl_dir+rel+'_ground-truth-files_dataset.csv')
        line_level_data = pd.read_csv(line_lvl_dir+rel+'_defective_lines_dataset.csv')
