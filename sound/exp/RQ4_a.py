...
def spearman(dict1, dict2):
    common_tokens = set(dict1.keys()).intersection(set(dict2.keys()))

    scores1 = [dict1[token] for token in common_tokens]
    scores2 = [dict2[token] for token in common_tokens]

    spearman_coefficient, p_value = scipy.stats.spearmanr(scores1, scores2)
...
def process_project_spearman(releases, base_dir, suffix):
    result = []
    for i in range(len(releases)):
        file1 = os.path.join(base_dir, f"{releases[i]}{suffix}")
        file2 = os.path.join(base_dir, f"{releases[i + 1]}{suffix}")
        if os.path.exists(file1) and os.path.exists(file2):
            dict1 = read_json_to_dict(file1)
            dict2 = read_json_to_dict(file2)
            spearman_coefficient, p_value = spearman(dict1, dict2)
            result.append((releases[i], releases[i + 1], spearman_coefficient, p_value))
    return result


def main():
    base_dir = '../Data/score/'
    suffixes = ["_barinel_normal.json"]
    results = []

    for project, releases in all_releases.items():
        for suffix in suffixes:
            results.extend(process_project_spearman(releases, base_dir, suffix))

    result_df = pd.DataFrame(results, columns=['Release1', 'Release2', 'Spearman', 'p-value'])
    result_df.to_csv('../Result/Discussion/barinel_spearman.csv', index=False)
