...
group_order <- c("Barinel", "Tarantula", "Ochiai", "Dstar", "GLANCE-LR", "Op2", "Ngram", "GLANCE-EA", "LineDP", "GLANCE-MD", "DeepLineDP", "ErrorProne")
...
group_colors <- c("Barinel" = "#F8766D", "Tarantula" = "#F8766D", "Ochiai" = "#F8766D",
                  ...
                  "LineDP" = "#00BFC4", "GLANCE-EA" = "#00BFC4", "GLANCE-MD" = "#00BFC4", "ErrorProne" = "#00BFC4", "Ngram" = "#00BFC4")
...
ggplot(data, aes(x = group, y = ifa)) +
  ...
  guides(fill=FALSE) +
  theme(axis.text.x=element_text(size = 24,angle = 30,hjust = 0.75,vjust = 0.75),
        axis.title.y=element_text(size=14),
        axis.text.y=element_text(size=24)) 
  

ggsave(paste('./result/RQ2_IFA.pdf',sep=""),width = 12,height = 6,dpi=200)


data <- data.frame()

group_order <- c("Barinel", "Tarantula", "Ochiai", "Dstar", "Op2", "GLANCE-LR", "LineDP", "GLANCE-EA", "GLANCE-MD", "Ngram", "DeepLineDP", "ErrorProne")
...
ggplot(data, aes(x = group, y = recall_20)) +
  ...
  theme(axis.text.x=element_text(size = 24,angle = 30,hjust = 0.75,vjust = 0.75),
        axis.title.y=element_text(size=14),
        axis.text.y=element_text(size=24))
ggsave(paste('./result/RQ2_recall.pdf',sep=""),width = 12,height = 6,dpi=200)


data <- data.frame()

group_order <- c("Barinel", "Tarantula", "Ochiai", "Dstar", "Op2", "LineDP", "GLANCE-LR", "GLANCE-EA", "GLANCE-MD", "Ngram",  "DeepLineDP", "ErrorProne")
...
group_colors <- c("Barinel" = "#F8766D", "Tarantula" = "#F8766D", "Ochiai" = "#F8766D",
                  "Dstar" = "#F8766D", "Op2" = "#F8766D", "DeepLineDP" = "#00BFC4","GLANCE-LR" = "#00BFC4",
                  "LineDP" = "#00BFC4", "GLANCE-EA" = "#00BFC4", "GLANCE-MD" = "#00BFC4", "Ngram" = "#00BFC4", "ErrorProne" = "#00BFC4")
...
ggplot(data, aes(x = group, y = effort)) +
  ...
  theme(axis.text.x=element_text(size = 24,angle = 30,hjust = 0.75,vjust = 0.75),
        axis.title.y=element_text(size=14),
        axis.text.y=element_text(size=24))
ggsave(paste('./result/RQ2_effort.pdf',sep=""),width = 12,height = 6,dpi=200)
